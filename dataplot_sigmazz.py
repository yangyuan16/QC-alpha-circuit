...
loadexp1 = 'H_heisenberg_sigmazz_t(%g,%g,%g)_Nq%d_Nc%d_hx%g' % (t1, t2, nt, Nbody, Nbody, hx_fix) + 'exact' + '.pr'
print(loadpath1 + loadexp1)
data_exact = bf.load_pr(loadpath1 + loadexp1)
sigmazz_exact = data_exact['sigma_zz']
...
loadexp2 = 'H_heisenberg_sigmazz_t(%g,%g,%g)_Nq%d_Nc%d_hx%g_dt%g_shots%g' % (t1,t2,nt,Nbody,Nbody,hx_fix,dt,shots) + 'trotter' + '.pr'
print(loadpath2 + loadexp2)
data_trotter = bf.load_pr(loadpath2 + loadexp2)
sigmazz_trotter = data_trotter['sigma_zz']
...
loadexp3 = 'H_heisenberg_sigmazz_thetamax(%g,%g,%g)_Nq%d_Nc%d_hx%g_shots%g' % (theta1,theta2,ntheta,Nbody,Nbody,hx_fix,shots) + 'advance' + '.pr'
print(loadpath3 + loadexp3)
data_advance =  bf.load_pr(loadpath3 + loadexp3)
sigmazz_advance = data_advance['sigma_zz']
theta_advance = data_advance['theta_max_range'] * hx_fix
#
L3_,=ax1_.plot(theta_advance[::4], sigmazz_advance[::4], label=r'$\alpha$-circuit', ls='--', lw=3, color='blue',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='blue', markerfacecolor='none')

L1,=ax1.plot(t_range_exact, sigmazz_exact, label=r'ED', ls='-', lw=5, color='red',
             marker='o', alpha=1, markersize=0, markeredgewidth=1.5, markeredgecolor='red', markerfacecolor='w')

L2,=ax1.plot(t_range_trotter[::4], sigmazz_trotter[::4], label=r'Trotter', ls='--', lw=3, color='orange',
...
legend1=plt.legend(handles=[L1,L2,L3_], loc = 4, bbox_to_anchor=(0.7, 0.58),ncol = 1,prop=legfont,markerscale=1,fancybox=None,shadow=None,frameon=False)
...
#=======================================================================================================================
label_x = r't'
label_y = r'$\sigma^z\sigma^z(t,h_x)$'
label_x_= r'$\theta$'
...
ax1.text(4,-0.03, r'$h_x = 0.5$', fontsize = 16, color='black')
...
ax1.text(8.1,0.3, r'$\sigma^z\sigma^z(\theta)$', fontsize = 16, color='black', rotation = 90)
#=============================================================
# 坐标轴设置第一层
...
L3_,=ax2_.plot(theta_advance[::4], sigmazz_advance[::4], label=r'$\alpha$-circuit', ls='--', lw=3, color='blue',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='blue', markerfacecolor='none')

L1,=ax2.plot(t_range_exact, sigmazz_exact, label=r'ED', ls='-', lw=5, color='red',
             marker='o', alpha=1, markersize=0, markeredgewidth=1.5, markeredgecolor='red', markerfacecolor='w')

L2,=ax2.plot(t_range_trotter[::4], sigmazz_trotter[::4], label=r'Trotter', ls='--', lw=3, color='orange',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='orange', markerfacecolor='w')
...
#==============================================================
ax2.text(0.3,-0.13, r'$\mathrm{(b)}$', fontsize=18)
#ax1.text(0.1,0.9, r'$\mathrm{perplexity = 18}$', fontsize=12)
# ax1.text(0.1,0.8, r'$\mathrm{n_{iter} = 5000}$', fontsize=12)
ax2.text(5,-0.12, r'$h_x = 0.8$', fontsize = 16, color='black')
#ax1.text(5.5,0.53, r'Heisenberg', fontsize = 15, color='black')
#ax1.text(5.5,0.5, r'S=1', fontsize = 15, color='black')
#ax1.text(5.5,0.53, r'uniform', fontsize = 15, color='black')
ax2.text(8.1,0.3, r'$\sigma^z\sigma^z(\theta)$', fontsize = 16, color='black', rotation = 90)
#=============================================================
# 坐标轴设置第一层
...
print(loadpath1 + loadexp1)
data_exact = bf.load_pr(loadpath1 + loadexp1)
c00 = np.array(data_exact['counts']['000']) + np.array(data_exact['counts']['001'])
c01 = np.array(data_exact['counts']['010']) + np.array(data_exact['counts']['011'])
c10 = np.array(data_exact['counts']['100']) + np.array(data_exact['counts']['101'])
c11 = np.array(data_exact['counts']['110']) + np.array(data_exact['counts']['111'])
sigmazz_exact = c00 - c01 - c10 + c11
...
print(loadpath2 + loadexp2)
data_trotter = bf.load_pr(loadpath2 + loadexp2)
c00_trotter = np.array(data_trotter['counts']['000']) + np.array(data_trotter['counts']['001'])
c01_trotter = np.array(data_trotter['counts']['010']) + np.array(data_trotter['counts']['011'])
c10_trotter = np.array(data_trotter['counts']['100']) + np.array(data_trotter['counts']['101'])
c11_trotter = np.array(data_trotter['counts']['110']) + np.array(data_trotter['counts']['111'])

sigmazz_trotter = c00_trotter - c01_trotter - c10_trotter + c11_trotter
...
c00_advance = np.array(data_advance['counts']['000']) + np.array(data_advance['counts']['001'])
c01_advance = np.array(data_advance['counts']['010']) + np.array(data_advance['counts']['011'])
c10_advance = np.array(data_advance['counts']['100']) + np.array(data_advance['counts']['101'])
c11_advance = np.array(data_advance['counts']['110']) + np.array(data_advance['counts']['111'])

sigmazz_advance = c00_advance - c01_advance - c10_advance + c11_advance
theta_range = data_advance['theta']

L3_,=ax3_.plot(theta_range[::2], sigmazz_advance[::2], label=r'$\alpha$-circuit', ls='--', lw=3, color='seagreen',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='seagreen', markerfacecolor='none')

L1,=ax3.plot(hx_range_exact, sigmazz_exact, label=r'ED', ls='-', lw=5, color='magenta',
             marker='o', alpha=1, markersize=0, markeredgewidth=1.5, markeredgecolor='magenta', markerfacecolor='w')

L2,=ax3.plot(hx_range_trotter[::4], sigmazz_trotter[::4], label=r'Trotter', ls='--', lw=3, color='darkviolet',
...
#label = ["Mg", "Mo", "Me","Ms"]
legfont = {'family' : 'Times New Roman','weight' : 'normal','size': 16, }###图例字体的大小###ncol 设置列的数量，使显示扁平化，当要表示的线段特别多的时候会有用
legend1=plt.legend(handles=[L1,L2,L3_], loc = 4, bbox_to_anchor=(0.98, -0.08),ncol = 1,prop=legfont,markerscale=1,fancybox=None,shadow=None,frameon=False)
#legend2=plt.legend(handles=[Le, Ls_a, Ls_b], loc = 4, bbox_to_anchor=(0.85, -0.05),ncol = 1,prop=legfont,markerscale=1,fancybox=None,shadow=None,frameon=False)
#plt.gca().add_artist(legend1)#####把图例legend1重新加载回来
## bty 图例框是否画出，o为画出，默认为n不画出
## markerscale 控制图例中maker的大小
...
#==============================================================
ax3.text(0.03,-0.13, r'$\mathrm{(c)}$', fontsize=18)
#ax1.text(0.1,0.9, r'$\mathrm{perplexity = 18}$', fontsize=12)
# ax1.text(0.1,0.8, r'$\mathrm{n_{iter} = 5000}$', fontsize=12)
ax3.text(0.2, 0.8, r'$t = 5$', fontsize = 16, color='black')
#ax1.text(5.5,0.53, r'Heisenberg', fontsize = 15, color='black')
#ax1.text(5.5,0.5, r'S=1', fontsize = 15, color='black')
#ax1.text(5.5,0.53, r'uniform', fontsize = 15, color='black')
ax3.text(1.03,0.3, r'$\sigma^z\sigma^z(\theta)$', fontsize = 16, color='black', rotation = 90)
#=============================================================
# 坐标轴设置第一层
...
L3_,=ax4_.plot(theta_range[::2], sigmazz_advance[::2], label=r'$\alpha$-circuit', ls='--', lw=3, color='seagreen',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='seagreen', markerfacecolor='none')

L1,=ax4.plot(hx_range_exact, sigmazz_exact, label=r'ED', ls='-', lw=5, color='magenta',
             marker='o', alpha=1, markersize=0, markeredgewidth=1.5, markeredgecolor='magenta', markerfacecolor='w')

L2,=ax4.plot(hx_range_trotter[::4], sigmazz_trotter[::4], label=r'Trotter', ls='--', lw=3, color='darkviolet',
             marker='o', alpha=1, markersize=10, markeredgewidth=1.5, markeredgecolor='darkviolet', markerfacecolor='w')
...
ax4.set_yticks([-0.2,0,0.5,1])

ax4_.set_xlim([0,8])
ax4_.set_ylim([-0.2,1])
ax4_.set_xticks([0,1.6,3.2,4.8,6.4,8])
#==============================================================
ax4.text(0.03,-0.13, r'$\mathrm{(d)}$', fontsize=18)
#ax1.text(0.1,0.9, r'$\mathrm{perplexity = 18}$', fontsize=12)
# ax1.text(0.1,0.8, r'$\mathrm{n_{iter} = 5000}$', fontsize=12)
ax4.text(0.4,-0.12, r'$t = 8$', fontsize = 16, color='black')
#ax1.text(5.5,0.53, r'Heisenberg', fontsize = 15, color='black')
#ax1.text(5.5,0.5, r'S=1', fontsize = 15, color='black')
#ax1.text(5.5,0.53, r'uniform', fontsize = 15, color='black')
ax4.text(1.03,0.3, r'$\sigma^z\sigma^z(\theta)$', fontsize = 16, color='black', rotation = 90)
#=============================================================
# 坐标轴设置第一层
...
#======================================================================
if ISsave:
    fig.savefig(r'.\Figs\fig3.eps',dpi=300, format='eps')
#===================================================
plt.show()
